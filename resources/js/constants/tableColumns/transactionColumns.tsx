...
export const transactionColumns = (t) => [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            if (!row.party_name || row.party_name === '-') return '-';
            const prefix = row.party_type === 'destination' ? `${t('ui.destination_label')}: `
                : row.party_type === 'source' ? `${t('ui.source_label')}: `
                : '';
            return `${prefix}${row.party_name}`;
        },
        className: 'text-center whitespace-nowrap',
    },
    {
