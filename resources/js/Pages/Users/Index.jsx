...
const RoleBadge = ({ role }) => {
  ...
  const roleColors = {
    ...
    'Warehouse Manager': 'bg-secondary text-secondary-foreground',
    'Branch Manager': 'bg-blue-500 text-white',
    'Cashier': 'bg-gray-500 text-white',
  };
  return (
    <span className={`px-3 py-1 text-xs font-semibold rounded-full inline-block ${roleColors[role] || 'bg-muted text-muted-foreground'}`}>
      {role}
    </span>
  );
};
      ...
      <AlertDialog open={confirmingUserDeletion !== null} onOpenChange={() => setConfirmingUserDeletion(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={deleteUser} className="bg-destructive hover:bg-destructive/90">Hapus Pengguna</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </IndexPageLayout>
