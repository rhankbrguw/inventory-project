...
import { ArrowRight, TrendingDown, TrendingUp, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function AdjustmentPreviewCard({ mode, onModeChange, currentStock, inputQty, unit, disabled }) {
    ...
    const diff = calcDiff();
    const isOverReduced = mode === 'reduction' && curr !== null && parsedInput > curr;
    const isZeroStockReduction = mode === 'reduction' && curr !== null && curr <= 0;
                ...
                <div className="p-3.5 rounded-lg border bg-muted/30 space-y-2.5">
                    ...
                    {isOverReduced && (
                        <p className="text-xs text-destructive font-medium flex items-center gap-1.5 pt-1">
                            ...
                            {t('ui.reduction_exceeds_stock')}
                        </p>
                    )}
                    {isZeroStockReduction && (
                        <p className="text-xs text-destructive font-medium flex items-center gap-1.5 pt-1">
                            <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
                            {t('ui.cannot_reduce_zero_stock')}
                        </p>
                    )}
                </div>
            )}
