...
export default function Adjust({ auth, products, locations, adjustmentReasons = [] }) {
    ...
    const parsedQty = parseFloat(data.quantity) || 0;
    const isReductionInvalid = data.mode === 'reduction' && (currentStock === null || currentStock <= 0 || parsedQty > currentStock || parsedQty <= 0);
    const isAdditionInvalid = data.mode === 'addition' && parsedQty <= 0;
    const isOpnameInvalid = data.mode === 'absolute' && (data.quantity === '' || parsedQty < 0);
    const isSubmitDisabled = processing || !isDirty || isFormDisabled || isReductionInvalid || isAdditionInvalid || isOpnameInvalid;
                ...
                <CardContent>
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <FormField label={t('ui.notes_label')} htmlFor="notes">
                            {adjustmentReasons.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mb-2">
                                    {adjustmentReasons.map((r: any) => {
                                        const reasonLabel = r.key ? (t(`ui.${r.key}`) || r.label) : r.label;
                                        return (
                                            <button
                                                type="button"
                                                key={r.key || r.value}
                                                disabled={isFormDisabled}
                                                onClick={() => setData('notes', reasonLabel)}
                                                className="px-2.5 py-1 text-xs rounded-md border border-border bg-muted/40 hover:bg-accent text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50"
                                            >
                                                {reasonLabel}
                                            </button>
                                        );
                        ...
                        <div className="flex items-center justify-end gap-4 pt-2">
                            ...
                            <Button disabled={isSubmitDisabled}>{t('ui.save')}</Button>
                        </div>
                    </form>
                </CardContent>
            </Card>
