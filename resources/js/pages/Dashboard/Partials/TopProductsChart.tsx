...
export default function TopProductsChart({ data, t }: { data?: any[]; t: any }) {
    const chartConfig = {
        total_qty: { label: t('ui.sold'), color: COLORS.CHART_2 }
    };
                            ...
                            <BarChart data={data} layout="vertical" margin={{ left: -10, right: 35, top: 4, bottom: 4 }}>
                                <defs>
                                    <linearGradient id="gradientTopBar" x1="0" y1="0" x2="1" y2="0">
                                        <stop offset="0%" stopColor={COLORS.CHART_2} stopOpacity={0.75} />
                                        <stop offset="100%" stopColor={COLORS.CHART_2} stopOpacity={1} />
                                    </linearGradient>
                                </defs>
                                <CartesianGrid horizontal={false} strokeDasharray="3 3" stroke={COLORS.BORDER_MUTED} />
                                <YAxis dataKey="name" type="category" tickLine={false} axisLine={false} width={95} fontSize={10} tick={{ fill: COLORS.MUTED_FOREGROUND }} />
                                <XAxis dataKey="total_qty" type="number" hide />
                                <ChartTooltip cursor={{ fill: COLORS.MUTED_30 }} content={({ active, payload }: any) => (!active || !payload?.length ? null : (
                                ...
                                <Bar dataKey="total_qty" fill="url(#gradientTopBar)" radius={[0, 6, 6, 0]} barSize={16} isAnimationActive animationDuration={900} animationEasing="ease-out" label={{ position: 'right', formatter: (numValue: any) => formatNumber(numValue), fontSize: 10, fill: COLORS.MUTED_FOREGROUND, fontWeight: 600 }} />
                            </BarChart>
                        </ChartContainer>
                    ) : (
