...
export default function SellCheckoutDialog({
    isOpen, onOpenChange, cartItems, totalPrice, locationId, customerId,
    targetLocationId, salesChannelId, paymentMethods, isInternal = false,
    buyerTab = 'general', customers = [], branches = [],
}) {
    ...
    const { data, setData, post, processing, errors, transform } = useForm({
        ...
        is_internal: isInternal, buyer_tab: buyerTab, sales_channel_id: salesChannelId || null,
        transaction_date: getNormalizedDate(), notes: '', payment_method_type_id: '',
        installment_terms: '1', interest_rate: '0', items: [],
    });

    const targetBranch = useMemo(() => branches.find((b) => b.id.toString() === targetLocationId?.toString()), [branches, targetLocationId]);
    const targetCustomer = useMemo(() => customers.find((c) => c.id.toString() === customerId?.toString()), [customers, customerId]);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setData({
                ...
                is_internal: isInternal, buyer_tab: buyerTab, sales_channel_id: salesChannelId,
                transaction_date: data.transaction_date || getNormalizedDate(), payment_method_type_id: paymentMethods[0]?.id.toString() || '',
                installment_terms: '1', interest_rate: '0',
                items: cartItems.map((i) => ({ product_id: i.product.id, quantity: i.quantity, sell_price: i.sell_price || i.product.price })),
            });
        }
    }, [isOpen, cartItems, locationId, customerId, targetLocationId, salesChannelId, isInternal, buyerTab]);
    ...
    const submit = (e) => {
        e.preventDefault();
        if (isInternal && !targetLocationId) return;
        if (buyerTab === 'customer' && !customerId) return;
        transform((d) => ({
            ...d, transaction_date: format(d.transaction_date, 'yyyy-MM-dd'),
            installment_terms: isCash ? 1 : parseInt(d.installment_terms),
            interest_rate: hasInstallment ? parseFloat(d.interest_rate) || 0 : 0,
            is_internal: isInternal, buyer_tab: buyerTab, idempotency_key: idempotencyKey,
        }));
                ...
                <form onSubmit={submit} className="space-y-4">
                    <div className="space-y-3 max-h-[65vh] overflow-y-auto px-1">
                        {Object.keys(errors).length > 0 && (
                            <div className="p-2.5 rounded-lg border border-destructive/20 bg-destructive/10 text-destructive text-xs space-y-1">
                                {Object.entries(errors).map(([field, msg]) => (
                                    <p key={field} className="flex items-center gap-1.5 font-medium">
                        ...
                        <div className="p-2.5 rounded-lg border bg-muted/40 text-xs flex items-center justify-between">
                            <span className="text-muted-foreground flex items-center gap-1.5 font-medium">
                                {isInternal ? <Building2 className="w-3.5 h-3.5" /> : <User className="w-3.5 h-3.5" />}
                                {isInternal ? t('ui.target_destination') : (customerId ? t('ui.customer') : t('ui.recipient'))}:
                            </span>
                            <span className="font-semibold text-foreground">
                                {isInternal ? (targetBranch?.name || '-') : (targetCustomer?.name || t('ui.walk_in_customer'))}
                            </span>
                        </div>
                        {isInternal && !targetLocationId && (
                            <p className="text-xs text-destructive flex items-center gap-1.5 font-medium">
                                <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
                                <span>{t('ui.dest_branch_required_warning')}</span>
                            </p>
                        )}
                        {buyerTab === 'customer' && !customerId && (
                            <p className="text-xs text-destructive flex items-center gap-1.5 font-medium">
                                <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
                    ...
                    <DialogFooter className="gap-2 sm:gap-0">
                        ...
                        <Button type="submit" disabled={processing || (isInternal && !targetLocationId) || (buyerTab === 'customer' && !customerId)} className="h-9 text-xs">{processing ? t('ui.processing') : t('ui.complete')}</Button>
                    </DialogFooter>
                </form>
            </DialogContent>
        </Dialog>
