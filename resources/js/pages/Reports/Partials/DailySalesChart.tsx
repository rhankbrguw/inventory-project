                                ...
                                <AreaChart data={data} margin={{ top: 12, right: 10, left: -15, bottom: 0 }}>
                                    <defs>
                                        <linearGradient id="gradientReportSales" x1="0" y1="0" x2="0" y2="1">
                                            <stop offset="5%" stopColor={COLORS.INFO} stopOpacity={0.32} />
                                    ...
                                    <XAxis dataKey="date" tickLine={false} axisLine={false} tickMargin={8} minTickGap={24} interval="preserveStartEnd" fontSize={11} tick={{ fill: COLORS.MUTED_FOREGROUND }} />
                                    <YAxis domain={[0, 'auto']} tickFormatter={(numValue: number) => `${(numValue / 1000000).toFixed(0)}M`} tickLine={false} axisLine={false} fontSize={11} tick={{ fill: COLORS.MUTED_FOREGROUND }} />
                                    <Tooltip cursor={{ stroke: COLORS.MUTED_FOREGROUND_35, strokeDasharray: '3 3' }} content={({ active, payload }: any) => (!active || !payload?.length ? null : (
                                    ...
                                    <Area type="monotone" dataKey="sales" stroke={COLORS.INFO} strokeWidth={2.4} fillOpacity={1} fill="url(#gradientReportSales)" dot={false} activeDot={{ r: 5, strokeWidth: 2, stroke: COLORS.BACKGROUND }} connectNulls isAnimationActive animationDuration={900} animationEasing="ease-out" />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    ) : (
