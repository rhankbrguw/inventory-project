...
class Handler extends ExceptionHandler
{
    ...
    {
        $this->renderable(fn (Throwable $e, $request) => $this->handleWebOrApiExceptions($e, $request));
        $this->renderable(fn (QueryException $e, $request) => $this->handleQueryExceptions($e, $request));
    }

    private function handleWebOrApiExceptions(Throwable $e, $request): mixed
    {
        ...
        if (($e instanceof AuthorizationException || ($e instanceof HttpException && $e->getStatusCode() === 403)) && ! $request->is('api/*')) {
            return \Inertia\Inertia::render('Errors/403', [
                'message' => $e->getMessage() ?: __('messages.http.403'),
            ])->toResponse($request)->setStatusCode(403);
        }
        if ($e instanceof \Symfony\Component\HttpKernel\Exception\MethodNotAllowedHttpException && ! $request->is('api/*')) {
            return redirect()->route('dashboard')->with('error', __('messages.http.default'));
        }

        return null;
    }

    private function handleQueryExceptions(QueryException $e, $request): mixed
    {
