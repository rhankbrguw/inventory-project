...
class SellCheckoutService
{
    ...
    public function processCheckout(User $user, array $validated): Sell
    {
        $idempotencyKey = $validated['idempotency_key'] ?? null;
        $lock = $idempotencyKey ? \Illuminate\Support\Facades\Cache::lock("sell_idemp_{$user->id}_{$idempotencyKey}", 15) : null;

        if ($lock && ! $lock->get()) {
            throw new \App\Exceptions\BusinessLogicException(__('messages.sell.duplicate_submission_in_progress'));
        }

        try {
            $total = collect($validated['items'])->sum(fn ($i) => $i['quantity'] * $i['sell_price']);
            $targetId = $validated['target_location_id'] ?? null;
            $terms = (int) ($validated['installment_terms'] ?? 1);
            $rate = $terms > 1 ? (float) ($validated['interest_rate'] ?? 0) : 0.0;
            $status = $targetId ? Sell::STATUS_PENDING_APPROVAL : Sell::STATUS_COMPLETED;

            return $this->createSell($user, $validated, $total, $targetId, $terms, $rate, $validated['sales_channel_id'] ?? null, $status);
        } finally {
            $lock?->release();
        }
    }
    ...
    private function processSellItems(Sell $sell, array $sellAttributes, string $status): void
    {
        ...
        $channelId = $sell->sales_channel_type_id;
