...
class SellCheckoutDataService
{
    ...
    private function getAccessibleLocations(User $user, ?array $accessibleIds): \Illuminate\Support\Collection
    {
        ...
        return $query->get()
            ...
            ->map(fn (Location $loc) => [
                'id' => $loc->id,
                'name' => $loc->name,
                'type_code' => $loc->type?->code,
                'role_at_location' => $user->getRoleCodeAtLocation($loc->id),
            ]);
    }
    ...
    public function getProductsForCheckout(User $user, ?int $locationId, ?string $search, string|int|null $typeId): LengthAwarePaginator
    {
        if (! $locationId) {
