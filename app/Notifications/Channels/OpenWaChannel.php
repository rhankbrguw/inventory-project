...
class OpenWaChannel
{
    ...
    private function dispatchHttpRequest(string $chatId, string $message): void
    {
        ...
        if (! $response->successful()) {
            ...
            $isInactive = str_contains($body, 'not active') || str_contains($body, 'stopped') || str_contains($body, 'not running') || $response->status() === 404;
            if ($isInactive) {
                Http::timeout(5)->withHeaders(['X-API-Key' => $apiKey])->post("{$baseUrl}/api/sessions/{$sessionId}/start");
                sleep(2);
                $response = Http::timeout(self::DEFAULT_TIMEOUT_SECONDS)->withHeaders(['X-API-Key' => $apiKey])->post($url, ['chatId' => $chatId, 'text' => $message]);
            }
        }
