...
class Product extends Model
{
    ...
    {
        $inv = $this->relationLoaded('inventories') ? $this->inventories->first() : null;

        return ($inv && $inv->selling_price > 0) ? (float) $inv->selling_price : (float) $this->price;
    }

    public function getEffectivePrice(?int $locationId): float
    {
        $inv = $locationId ? $this->inventories->where('location_id', $locationId)->first() : null;

        return ($inv && $inv->selling_price > 0) ? (float) $inv->selling_price : (float) $this->price;
    }

    public function getPriceForChannel(?int $channelId, ?int $locationId = null): float
    {
        if ($channelId && $locationId) {
            $inv = $this->inventories->where('location_id', $locationId)->first();
            if (! empty($inv->channel_prices_override[$channelId])) {
                return (float) $inv->channel_prices_override[$channelId];
            }
        }
        $channelPrice = $channelId ? $this->prices->where('type_id', $channelId)->first() : null;

        return ($channelPrice && $channelPrice->price !== null) ? (float) $channelPrice->price : $this->getEffectivePrice($locationId);
    }
