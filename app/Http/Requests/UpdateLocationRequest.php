      ...
      $validator->after(function ($validator) {
         ...
         foreach ($assignments as $index => $assignment) {
            ...
            if ($isWarehouse && $role->name === 'Branch Manager') {
               $validator->errors()->add("assignments.{$index}.role_id", 'Peran Branch Manager tidak valid untuk tipe Gudang.');
            }

            if ($isBranch && $role->name === 'Warehouse Manager') {
               $validator->errors()->add("assignments.{$index}.role_id", 'Peran Warehouse Manager tidak valid untuk tipe Cabang.');
            }
         }
      });
   }
