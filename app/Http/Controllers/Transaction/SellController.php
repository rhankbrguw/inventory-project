...
class SellController extends Controller
{
    ...
    public function store(StoreSellRequest $request): RedirectResponse
    {
        ...
        try {
            $this->checkoutService->processCheckout($request->user(), $validated);
        ...
        return Redirect::route('transactions.index', ['tab' => 'sells'])->with('success', __('messages.sell.created'));
    }
    ...
    public function show(Sell $sell): Response
    {
        ...
        $sell->load([
            ...
            'items.product' => fn ($q) => $q->withTrashed()->withoutGlobalScopes(), 'items.salesChannel',
            'stockMovements.product' => fn ($q) => $q->withTrashed()->withoutGlobalScopes(), 'stockMovements.salesChannel',
        ]);
