...
class SellCartController extends Controller
{
    ...
    private function resolveEffectivePrice(int $productId, int $locationId, ?int $channelId): float
    {
        ...
        return $product->getPriceForChannel($channelId, $locationId);
    }
    ...
    private function addOrUpdateCartItem(int $userId, array $cartItemPayload, float $effectivePrice): void
    {
        $item = SellCartItem::where('user_id', $userId)->where('location_id', $cartItemPayload['location_id'])
            ->where('product_id', $cartItemPayload['product_id'])->first();

        if ($item) {
            $item->increment('quantity', $cartItemPayload['quantity']);
            $item->update(['sell_price' => $effectivePrice, 'sales_channel_type_id' => $cartItemPayload['sales_channel_id'] ?? null]);
        } else {
            SellCartItem::create([
    ...
    public function updatePricesByChannel(Request $request): RedirectResponse
    {
        ...
        $channelId = $validated['sales_channel_id'] ?? null;
        $locationId = (int) $validated['location_id'];

        $items = $request->user()->sellCartItems()
            ->where('location_id', $locationId)
            ->with(['product.inventories', 'product.prices'])
            ->get();

        foreach ($items as $item) {
            if ($item->product) {
                $item->update([
                    'sell_price' => $item->product->getPriceForChannel($channelId, $locationId),
