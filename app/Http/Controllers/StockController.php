...
class StockController extends Controller
{
    ...
    public function showAdjustForm(): Response
    {
        ...
        return Inertia::render('Stock/Adjust', [
            ...
            'adjustmentReasons' => [
                ['key' => 'input_correction', 'value' => __('ui.input_correction'), 'label' => __('ui.input_correction')],
                ['key' => 'physical_stock_adjustment', 'value' => __('ui.physical_stock_adjustment'), 'label' => __('ui.physical_stock_adjustment')],
                ['key' => 'damaged_goods', 'value' => __('ui.damaged_goods'), 'label' => __('ui.damaged_goods')],
                ['key' => 'returned_goods', 'value' => __('ui.returned_goods'), 'label' => __('ui.returned_goods')],
            ],
        ]);
    }
    ...
    public function adjust(AdjustStockRequest $request): RedirectResponse
    {
        ...
        if ($user->cannot('adjust', [Inventory::class, $locationId])) {
            return Redirect::back()->with('error', __('messages.access_denied_location'));
        }

        $accessibleIds = $user->getAccessibleLocationIds();
        if ($accessibleIds && ! in_array($locationId, $accessibleIds)) {
            return Redirect::back()->with('error', __('messages.access_denied_location'));
        }
