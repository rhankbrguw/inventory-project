...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request): Response|RedirectResponse
    {
        ...
        if ($user->level !== Role::LEVEL_SUPER_ADMIN) {
            $validIds = array_values(array_filter($accessibleIds ?? [], fn ($id) => $id > 0));
            if (empty($selectedLocation) || $selectedLocation === 'all' || ! in_array((int) $selectedLocation, $validIds, true)) {
                $selectedLocation = ! empty($validIds) ? (string) $validIds[0] : null;
            }
        }
