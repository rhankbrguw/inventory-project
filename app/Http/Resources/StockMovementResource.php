...
class StockMovementResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'notes' => $this->resolveLocalizedNotes(),
    ...
    private function resolveOriginDestinationInfo(): ?array
    {
        if (! $this->relationLoaded('reference') || ! $this->reference) {
            return $this->notes ? ['type' => 'notes', 'label' => __('ui.notes'), 'name' => $this->resolveLocalizedNotes()] : null;
        }
        ...
        return match (true) {
            ...
            $this->reference instanceof User => ['type' => 'pic', 'label' => __('ui.pic'), 'name' => $this->reference->name.' - '.$this->resolveLocalizedNotes()],
    ...
    private function resolveLocalizedNotes(): string
    {
        ...
        $idTerms = ['(Tambah:', '(Rusak/Kurang:', 'Koreksi input', 'Barang rusak', 'Barang telah diterima di lokasi tujuan.', 'Barang dalam proses pengiriman.', 'Dikirim ke ', 'Diterima dari '];
        $enTerms = ['(Addition:', '(Damaged/Reduction:', 'Input correction', 'Damaged goods', 'Goods received at destination.', 'Goods in transit.', 'Shipped to ', 'Received from '];

        return App::isLocale('en') ? str_ireplace($idTerms, $enTerms, $notes) : str_ireplace($enTerms, $idTerms, $notes);
    }
}
